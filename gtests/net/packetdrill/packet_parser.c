		     ...
		     u8 *packet_end, char **error)
{
	...
	if (psp_header_len + PSP_TRL_SIZE > psp_bytes) {
		asprintf(error, "PSP encap overflows packet");
		goto error_out;
	}
	...
	struct header *psp_header =
		packet_append_header(packet, HEADER_PSP, psp_header_len);
	if (psp_header == NULL) {
	...
	if (psp->next_header == IPPROTO_IPIP || psp->next_header == IPPROTO_IPV6) {
		asprintf(error, "PSP tunnel mode not supported");
		goto error_out;
	}
		     ...
		     u8 *packet_end, char **error)
{
	...
	if (is_psp_port(ntohs(udp->dst_port)))
